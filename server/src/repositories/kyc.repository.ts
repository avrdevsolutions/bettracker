...
export class KycRepository {
    ...
    async findSubmittedUsers() {
        const results = await pool.query(
            `SELECT u.id, u.email, u.name, u.full_name, u.kyc_status, u.created_at,
                    json_agg(json_build_object('id', d.id, 'doc_type', d.doc_type, 'status', d.status, 'created_at', d.created_at)) AS documents
             FROM users u
